...
class Test(unittest.TestCase):
    ...
    def test(self):
        ...
        production.click('reset_bom',
                         change=[
                             'bom', 'product', 'uom', 'quantity', 'inputs',
                             'outputs', 'company', 'warehouse', 'location'
                         ])
        self.assertEqual(
            sorted([i.quantity for i in production.inputs]), [10, 300])
        output, = production.outputs
        ...
        with config.set_context(locations=[storage.id]):
            self.assertEqual(Product(product.id).quantity, 2)

        # Check Analysis Creation Real and deviation movements
        analysis = production.production_cost_analysis
        self.assertEqual(len(analysis.real_inputs_costs), 2)
        self.assertEqual(len(analysis.real_outputs_costs), 1)
        self.assertEqual(len(analysis.input_deviation_costs), 2)
        self.assertEqual(len(analysis.output_deviation_costs), 1)
        ...
        production.reload()
        output, = production.outputs
        self.assertEqual(output.effective_date, yesterday)
        ...
        production.bom = bom
        production.quantity = 2
        self.assertEqual([i.quantity for i in production.inputs], [0.0, 0.0])
        output, = production.outputs
        self.assertEqual(output.quantity, 0.0)
